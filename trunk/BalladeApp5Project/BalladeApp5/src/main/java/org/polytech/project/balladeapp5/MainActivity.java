...
import org.polytech.project.balladeapp5.KMLParser.Placemark;
    ...
    public static List<Mesh> updateMesh(List<String> coordList)
    {
        ...
        for(String current : coordList)
        {
            ...
            for(String currentVertex : verticesList)
            {
                ...
                if(isOk == 0)
                {
                    Vector3 test = new Vector3(Double.parseDouble(list.get(0)), Double.parseDouble(list.get(1)), Double.parseDouble(list.get(2)));
